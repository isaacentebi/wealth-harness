...
class _ConsentGate:
    """The consent gate of one server: what a save would change (its subject) and whether the person agreed.

    It holds the server's service, the turn's consent evidence and the one-time confirmation codes;
    ``build_server`` binds its methods to the names its tools call.
    """

    def __init__(self, service: WealthService, turn: _consent.Turn,
                 confirmations: _consent.Confirmations) -> None:
        self.service = service
        self.turn = turn
        self.confirmations = confirmations

    def require(self, kind: str, allowed: bool, subject: tuple[Any, str] | None = None,
        ...
        if self.turn.host_handles or subject is None:
        ...
        if confirm is True:
            if self.confirmations.redeem(digest, code, client):
                return None
        ...
        return {
            "status": "needs_person",
            "result": {"summary": summary, "confirmation_code": self.confirmations.issue(digest, client),
    ...
    def ingest_subject(self, client_id: str, action: str, inputs: Mapping[str, Any]) -> tuple[Any, str] | None:
        state = self.service._ingest_state(client_id)
    ...
    def placed_subject(self, client_id: str, inputs: Mapping[str, Any]) -> tuple[Any, str] | None:
        ...
        with WealthStore(self.service.db_path) as store:
    ...
    def decision_subject(self, client_id: str, inputs: Mapping[str, Any]) -> tuple[Any, str] | None:
        decision_id = inputs.get("decision_id")
        decisions = self.service.inspect(client_id).get("decisions") or []
    ...
    def contradiction_subject(self, client_id: str, contradiction_id: str, choice: str,
                                    valid_from: str | None) -> tuple[Any, str] | None:
        pending = self.service.contradictions(client_id).get("contradictions") or []
    ...
    def document_figures(self, client_id: str, matched: list[dict]) -> set[float]:
    ...
    def vet_facts(self, client_id: str, facts: list[dict]) -> list[str]:
        ...
        for fact in facts:
            ...
            if kind == "document":
                if sources is None:
                    sources = self.service.ingested_sources(client_id)
                ...
                if fact.get("value") is not None:
                    missing = _consent.ungrounded(fact["value"], self.document_figures(client_id, matched))
                    if missing:
                        ...
                        warnings.append(f"{key}: saved as inferred, not as the statement's figure: the statement's "
                                        ...
                                        "on it")
            elif kind == "user" and self.turn.session is not None and fact.get("value") is not None:
                ...
                if current is None:
                    snapshot = self.service.inspect(client_id, keys=[f["key"] for f in facts])
...
def build_server(db_path: str | None = None, *, include_behavior: bool = False,
    ...
    gate = _ConsentGate(service, turn, confirmations)
    require = gate.require
    ingest_subject = gate.ingest_subject
    placed_subject = gate.placed_subject
    decision_subject = gate.decision_subject
    contradiction_subject = gate.contradiction_subject
    vet_facts = gate.vet_facts
