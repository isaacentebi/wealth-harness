...
def _install_capture() -> None:
    ...
    if getattr(agent._run_process, "_eval_capture", False):
        return
    original = agent._run_process

    def capturing(command, prompt, timeout):
        code, stdout = original(command, prompt, timeout)
        _captured.stdout = stdout
        return code, stdout

    capturing._eval_capture = True  # type: ignore[attr-defined]
    agent._run_process = capturing
...
def run_scenario(data: dict, scenario: dict, options: argparse.Namespace) -> dict[str, Any]:
    ...
    events = agent.parse_events(getattr(_captured, "stdout", "") or "")
    return {
