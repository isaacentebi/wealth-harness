...
| `web.py`, `chat.html`, `profile.html`, `static/` | Loopback browser chat, the You page, uploads, onboarding cards, order cards; the chat page's CSS and JS are `static/chat.css` and `static/chat.js`, served by exact name |
