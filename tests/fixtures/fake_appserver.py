...
Behaviour comes from ``FAKE_APPSERVER_MODE`` (normal, slow, ingest, commentary, broken, leaky, failed).
...
record = {"pid": os.getpid(), "argv": sys.argv[1:], "codex_home": os.environ.get("CODEX_HOME"),
          "env": sorted(os.environ), "requests": []}
    ...
    for raw in sys.stdin:
        ...
        if method == "initialize":
            ...
            send({"id": request_id, "result": {"config": {
                "mcp_servers": servers, "sandbox_mode": override("sandbox_mode"),
