...
def test_live_run_path_with_a_fake_codex_process(tmp_path, monkeypatch):
    ...
    def fake_process(command, prompt, timeout):
    ...
    monkeypatch.setattr(agent, "_run_process", fake_process)
    ...
    assert "no saved facts" in prompts[0][1]
    ...
    assert "already has saved facts" in prompts[1][1]
